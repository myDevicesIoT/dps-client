    ...
    if [ "$UPDATE_FILE_PATH" != "/tmp/" ]; then
        ...
        if ( test "$RESULT" == "60" || test "$RESULT" == "77" ) && ! test -f /etc/ssl/certs/ca-certificates.crt; then
            ...
            if command -v update-ca-certificates &> /dev/null; then
                ...
                opkg install ca-certificates >> $LOG_FILE 2>&1
                opkg upgrade ca-certificates >> $LOG_FILE 2>&1
            fi
            curl -s -f $UPDATE_URL --output $UPDATE_FILE_PATH
            RESULT=$?
        fi
        ...
        if [ $DOWNLOAD_MD5 == $UPDATE_MD5 ]; then
            ...
            echo "opkg install $UPDATE_FILE_PATH $UPDATE_OPTIONS" >> $LOG_FILE 2>&1
            opkg install $UPDATE_FILE_PATH $UPDATE_OPTIONS >> $LOG_FILE 2>&1
        else
